...
static int buddy_test()
{
    ...
    //for (int i = 30; i >= 23; --i) {
    //    while (!error) {
    //        error = vka_alloc_untyped(vka, i, &ut_container);
    //    }
    //    //printf("Clean up all %d size untyped object\n", i);
    //    error = 0;
    //}
    //for (int i = 0; i < 2; ++i) {
    //    error = vka_alloc_untyped(vka, 22, &ut_container);
    //}
	...
	for(int i = 0; i < ITER; i++)
	{
		...
		time = get_random_time(5000);

        mem[i].time = time + i;
        mem[i].frame_num = size;		//add

        printf("alloc_block{size: %d, time: %d}\n", CTZL(size) + 12, time);

        if (bitmap_tree_test) {
            //error = vka_alloc_frame_contiguous(vka, CTZL(size) + 12, &mem[i].frame_num, &mem[i].blk);
